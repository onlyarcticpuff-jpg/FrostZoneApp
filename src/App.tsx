...
/* ─── TONAPI NFT collections ─────────────────────────────────────
   Filters to only show verified Telegram NFTs:
   - Telegram sticker / gift collections
   - Collectible usernames (Fragment)
   - Known verified collections only — no scam or shitpost NFTs
──────────────────────────────────────────────────────────────── */
 ...
 *  Extend this list as new official collections launch. */
const VERIFIED_ADDRESSES = new Set([
  // Fragment collectible usernames contract
  "EQC3dNlesgVD8YbAazcauIrXBPfiVhMMr5YYk2in0Mtsz0Bh",
  // Telegram anonymous numbers
  "EQAVGhk_3rUA3ypZAZ1SkVGZIns4woSzSDBHLHMGRqos9zvN",
  // Telegram sticker gifts (official)
  "EQDo-ybApBOCdGVTZkPYMO9R49vkJxR2gZ_k9kkgxHEQbfNb",
  // TON DNS (.ton domains)
  "EQC3dNlesgVD8YbAazcauIrXBPfiVhMMr5YYk2in0Mtsz0Bh",
  // Getgems verified — add more as needed
]);

/** Heuristic: accept a collection if it looks legitimate.
 *  Rejects if name is missing, suspiciously short, or contains
 *  common scam/shitpost signals. */
function isLikelyLegit(c: any): boolean {
  const name: string = (c.metadata?.name ?? "").trim();
  if (!name || name.length < 3) return false;

  // Must have a cover image
  const hasImage = !!(c.previews?.length);
  if (!hasImage) return false;

  // Must have meaningful owner count (filters brand-new / fake drops)
  const owners: number = c.owner_count ?? 0;
  if (owners < 50) return false;

  // Basic scam-keyword filter
  const low = name.toLowerCase();
  const scamKeywords = ["airdrop", "free ", "giveaway", "claim", "mint now", "presale", "rug", "100x", "elon", "pepe", "shib", "doge"];
  if (scamKeywords.some((kw) => low.includes(kw))) return false;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const filtered = raw
          .filter((c) => VERIFIED_ADDRESSES.has(c.address) || isLikelyLegit(c))
          .slice(0, 6)
          .map((c) => ({
            address:    c.address,
...
function NftKindBadge({ kind }: { kind: "gift" | "username" | "verified" }) {
  if (kind === "gift") return (
    <span className="nft-badge nft-badge--gift">
      <Gift size={8} strokeWidth={2.5} /> Gift
    </span>
  );
  if (kind === "username") return (
    <span className="nft-badge nft-badge--username">
      <AtSign size={8} strokeWidth={2.5} /> Name
    </span>
  );
  return (
    <span className="nft-badge nft-badge--verified">
      <BadgeCheck size={8} strokeWidth={2.5} /> Verified
    </span>
  );
}
    ...
    <div className="app-shell">
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Each card is a fixed-height flex column */
        .nft-card {
          display: flex;
          flex-direction: column;
          border-radius: 12px;
          ...
          height: 148px;       /* fixed total card height */
        }
        .nft-card:active { transform: scale(0.97); }

        /* Square image fills the top portion */
        .nft-img-wrap {
          width: 100%;
        ...
        /* Bottom info area — fixed height, never grows */
        .nft-info {
          flex: 1;
          min-height: 0;
          padding: 5px 6px 5px;
        ...
        .nft-badge--verified { background: rgba(52,211,153,0.12); color: var(--green); }
            ...
            <GlassCard accent="#a78bfa">
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                  ...
                  <span className="cap" style={{ color:"var(--green)", fontWeight:600 }}>Verified only</span>
                </div>
              </div>
              ...
              <div style={{ display:"flex", gap:6, marginTop:8 }}>
                ...
                <span className="nft-badge nft-badge--verified"><BadgeCheck size={8} strokeWidth={2.5} /> Verified</span>
              </div>
                  ...
                  {nfts.map((col) => (
                    ...
                    >
                      ...
                      <div className="nft-info">
                        ...
                        {col.ownerCount != null && (
                          <span className="nft-owners">
                            {col.ownerCount.toLocaleString()} owners
                          </span>
                        )}
                      </div>
                    </a>
              ...
              {!nftLoading && !nftError && nfts.length === 0 && (
                <p className="cap" style={{ textAlign:"center", padding:"16px 0" }}>
                  No verified collections found
                </p>
              )}
            </GlassCard>

            {/* ── Explore TON ecosystem links ── */}
            <GlassCard accent="#60a5fa">
              ...
              <div className="ton-links">
                <a
                  className="ton-link-btn"
                  href="https://fragment.com"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <AtSign size={12} strokeWidth={2.5} style={{ color:"var(--blue)", flexShrink:0 }} />
                  Fragment
                </a>
                <a
                  className="ton-link-btn"
                  ...
                  rel="noopener noreferrer"
                >
                  <Gem size={12} strokeWidth={2.5} style={{ color:"var(--purple)", flexShrink:0 }} />
                  Getgems
                </a>
                <a
                  className="ton-link-btn"
                  ...
                  rel="noopener noreferrer"
                >
                  <Wallet size={12} strokeWidth={2.5} style={{ color:"var(--green)", flexShrink:0 }} />
                  TG Wallet
                </a>
                <a
                  className="ton-link-btn"
                  ...
                  rel="noopener noreferrer"
                >
                  <Layers size={12} strokeWidth={2.5} style={{ color:"var(--orange)", flexShrink:0 }} />
                  TONScan
                </a>
              </div>
            </GlassCard>
