...
// ─── Rarity Config ─────────────────────────────────────────────────────────────
const RARITY: Record<Rarity, { label: string; color: string; bg: string; border: string; icon: React.ReactNode }> = {
  consumer:   { label: "Consumer",   color: "#b0c3d9", bg: "rgba(176,195,217,0.08)", border: "rgba(176,195,217,0.15)", icon: <Package size={10} /> },
  industrial: { label: "Industrial", color: "#5e98d9", bg: "rgba(94,152,217,0.08)",  border: "rgba(94,152,217,0.15)",  icon: <Shield size={10} /> },
  milspec:    { label: "Mil-Spec",   color: "#4b69ff", bg: "rgba(75,105,255,0.08)",  border: "rgba(75,105,255,0.15)",  icon: <Zap size={10} /> },
  restricted: { label: "Restricted", color: "#8847ff", bg: "rgba(136,71,255,0.08)",  border: "rgba(136,71,255,0.15)",  icon: <Star size={10} /> },
  classified: { label: "Classified", color: "#d32ce6", bg: "rgba(211,44,230,0.08)",  border: "rgba(211,44,230,0.15)",  icon: <Gem size={10} /> },
  covert:     { label: "Covert",     color: "#eb4b4b", bg: "rgba(235,75,75,0.08)",   border: "rgba(235,75,75,0.15)",   icon: <Crown size={10} /> },
  contraband: { label: "Contraband", color: "#e4ae39", bg: "rgba(228,174,57,0.10)",  border: "rgba(228,174,57,0.18)",  icon: <Crown size={10} /> },
};

// ─── Compact Case Grid Card ────────────────────────────────────────────────────
function CaseGridCard({ item, index, onClick }: { item: CaseType; index: number; onClick: () => void }) {
  ...
  const press = () => {
    ...
    cardRef.current.style.transform = "scale(0.94)";
  };
  const release = () => {
    if (!cardRef.current) return;
    cardRef.current.style.transition = "transform 0.32s cubic-bezier(0.34,1.56,0.64,1)";
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: `fadeUp 0.38s cubic-bezier(0.34,1.56,0.64,1) ${index * 0.06}s both`,
        willChange: "transform",
        display: "flex",
        flexDirection: "column",
      }}
    >
      {/* Image stage — full bleed, bold */}
      <div style={{
        position: "relative",
        aspectRatio: "1 / 1",
        background: "#0a0a0a",
        ...
        <div style={{
          ...
          background: `radial-gradient(ellipse 90% 90% at 50% 55%, ${glowColor}, transparent 68%)`,
        }} />
        {/* Case image — fills the whole square */}
        <img
          src={item.image_url}
          alt={item.name}
          style={{
            ...
            padding: "12%",
            zIndex: 1,
            filter: `drop-shadow(0 10px 28px ${glowColor}) drop-shadow(0 2px 8px rgba(0,0,0,0.9))`,
            animation: "floatCase 4s ease-in-out infinite",
          }}
        />
        {/* Soft bottom fade into info row */}
        <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {item.currency === "TON"
            ? <img src={TonIcon} style={{ width: 12, height: 12 }} />
            : <img src={StarsIcon} style={{ width: 12, height: 12, filter: "brightness(0) saturate(100%) invert(79%) sepia(96%) saturate(649%) hue-rotate(355deg) brightness(101%) contrast(105%)" }} />
          }
          {item.price}
        </div>
      </div>
...
function CaseDetailPage({ caseData, onClose }: { caseData: CaseType; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    // Prevent body scroll while this page is open
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          border: `1px solid ${glowColor.replace(/[\d.]+\)$/, "0.22)")}`,
      ...
      <div style={{
        ...
        <div style={{
          position: "absolute", inset: 0, zIndex: 0,
          background: `radial-gradient(ellipse 90% 80% at 50% 60%, ${glowColor}, transparent 68%)`,
        }} />
        <CaseOpeningCanvas
      ...
      <div style={{ padding: "12px 16px 0", flexShrink: 0 }}>
        <button
          style={{
            ...
            cursor: "pointer",
            fontFamily: "inherit",
            boxShadow: `0 4px 28px rgba(255,255,255,0.12), 0 0 0 1px rgba(255,255,255,0.06)`,
      ...
      {/* ── What's Inside — header is static, ONLY the grid scrolls ── */}
      <div style={{
        flex: 1,
        display: "flex",
        flexDirection: "column",
        ...
        {/* Scrollable grid container — only THIS scrolls */}
        <div style={{
          flex: 1,
          overflowY: "auto",
          ...
          {loadingItems ? (
            ...
            <div style={{
              ...
              fontSize: 13, padding: "32px 0", fontWeight: 600,
            }}>
                    ...
                    <div style={{
                      ...
                      {item.image_url
                        ? <img
                          ...
                          />
                        : <Package size={22} color={cfg.color} opacity={0.6} />
                      }
                      {/* Rarity top strip */}
                      <div style={{
    ...
    <div className="app">
      <style>{`
        *, *::before, *::after {
        ...
        .balance-row { display: flex; gap: 7px; margin-bottom: 14px; }
        ...
        .skeleton-card {
          aspect-ratio: 1 / 1.25; border-radius: 18px;
