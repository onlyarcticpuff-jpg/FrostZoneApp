...
// Returns the Telegram.WebApp object if running inside the mini-app, else null.
const tgApp  = (): any => (window as any).Telegram?.WebApp ?? null;
// Returns the current Telegram user object from initDataUnsafe, else null.
...
// Only the fields that actually exist in the Supabase `users` table.
type UserProfile = {
  id:          string;
  telegram_id: number;
  created_at:  string;
...
// user_items row joined with case_items details.
type InventoryItem = {
  id:           string;   // user_items.id
  item_id:      string;
  obtained_at:  string;
  name:         string;   // from case_items
  rarity:       Rarity;
  image_url?:   string;
};
...
function InventoryPage() {
  ...
  useEffect(() => {
    ...
    // Join user_items → case_items to get name, rarity, image_url
    supabase
      .from("user_items")
      .select(`
        id,
        item_id,
        obtained_at,
        case_items ( name, rarity, image_url )
      `)
    ...
    <div style={{ animation: "fadeUp 0.38s cubic-bezier(0.34,1.56,0.64,1) both" }}>
      ...
      {!loading && items.length === 0 && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center",
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: "rgba(255,255,255,0.85)", marginBottom: 5 }}>
            No items yet
          </div>
...
function ProfilePage({ tonAddress }: { tonAddress: string }) {
  ...
  // ── Telegram SDK: source of truth for identity ──
  const tg          = tgUser();
  const displayName = [tg?.first_name, tg?.last_name].filter(Boolean).join(" ") || "Anonymous";
  const username    = tg?.username as string | undefined;
  // Telegram WebApp does NOT expose photo_url in initDataUnsafe; we can try it and fall back gracefully.
  const avatarUrl   = tg?.photo_url as string | undefined;
  const initials    = displayName.split(" ").map((w: string) => w[0]).slice(0, 2).join("").toUpperCase();

  // ── Supabase: only read created_at (and cases_opened if you have it) ──
  useEffect(() => {
    if (!tg?.id) { setLoading(false); return; }
    supabase
      .from("users")
  ...
  return (
    <div style={{ animation: "fadeUp 0.38s cubic-bezier(0.34,1.56,0.64,1) both" }}>
      ...
      {/* ── Hero Card ── */}
      <div style={{ borderRadius: 24, padding: "30px 20px 24px",
        background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.10)",
        boxShadow: CARD_SHADOW_DEEP, marginBottom: 12,
        ...
        <div style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)",
          width: "65%", height: 1,
          background: "linear-gradient(90deg, transparent, rgba(42,171,238,0.55), transparent)" }} />
        ...
        <div style={{ position: "relative", marginBottom: 16 }}>
          {loading ? (
            ...
            <img src={avatarUrl} alt={displayName} style={{ width: 92, height: 92, borderRadius: "50%",
              objectFit: "cover", border: "3px solid rgba(255,255,255,0.12)", display: "block",
              boxShadow: "0 6px 28px rgba(0,0,0,0.45), 0 0 0 1px rgba(255,255,255,0.07)" }} />
          ) : (
            <div style={{ width: 92, height: 92, borderRadius: "50%",
              ...
              boxShadow: "0 6px 28px rgba(42,171,238,0.32), 0 0 0 1px rgba(255,255,255,0.07)" }}>
              {initials || <User size={32} color="rgba(255,255,255,0.9)" />}
            </div>
          )}
          {/* Online dot */}
          <div style={{ position: "absolute", bottom: 4, right: 4, width: 15, height: 15,
            borderRadius: "50%", background: "#4CAF50", border: "2.5px solid #1c1c1e",
            boxShadow: "0 0 8px rgba(76,175,80,0.65)" }} />
        </div>

        {/* Name */}
        {loading
          ? <div style={{ width: 130, height: 24, borderRadius: 8, background: "rgba(255,255,255,0.07)",
              marginBottom: 6, animation: "shimmerPulse 1.6s ease-in-out infinite" }} />
          : <div style={{ fontSize: 23, fontWeight: 800, letterSpacing: "-0.7px", color: "#fff",
              textAlign: "center", lineHeight: 1.15 }}>{displayName}</div>
        }

        {/* @username — from Telegram SDK */}
        {!loading && username && (
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.38)", fontWeight: 500, marginTop: 4, letterSpacing: "-0.1px" }}>
            @{username}
          </div>
        )}

        {/* Joined — from Supabase created_at */}
        {!loading && joinedAt && (
          <div style={{ marginTop: 14, display: "flex", alignItems: "center", gap: 5,
            ...
            fontSize: 11, color: "rgba(255,255,255,0.42)", fontWeight: 600 }}>
            <Clock size={10} color="rgba(255,255,255,0.32)" />
            Joined {joinedAt}
          </div>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
        {/* Cases opened */}
        <div style={{ borderRadius: 18, padding: "16px 16px 14px",
          background: "rgba(255,255,255,0.055)", border: "1px solid rgba(255,255,255,0.09)",
          ...
          <div style={{ width: 36, height: 36, borderRadius: 11,
            ...
            display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Box size={16} color="#2AABEE" />
          </div>
          <div>
            {loading
              ? <div style={{ width: 48, height: 22, borderRadius: 6, background: "rgba(255,255,255,0.08)",
                  animation: "shimmerPulse 1.6s ease-in-out infinite" }} />
              : <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-0.7px", color: "#fff", lineHeight: 1 }}>
                  {casesOpened.toLocaleString()}
                </div>
            }
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", fontWeight: 600, marginTop: 4 }}>Cases Opened</div>
          </div>
        </div>
        ...
        <div style={{ borderRadius: 18, padding: "16px 16px 14px",
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ width: 36, height: 36, borderRadius: 11,
              ...
              display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Wallet size={16} color={tonAddress ? "#2AABEE" : "rgba(255,255,255,0.35)"} />
            </div>
            {tonAddress && (
              <button onClick={handleCopy} style={{ width: 28, height: 28, borderRadius: 9, flexShrink: 0,
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "-0.2px", lineHeight: 1,
              color: tonAddress ? "#fff" : "rgba(255,255,255,0.38)",
              fontFamily: tonAddress ? "'SF Mono','Fira Code',monospace" : "inherit",
              overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {tonAddress ? shortAddr : "Not connected"}
            </div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", fontWeight: 600, marginTop: 4 }}>TON Wallet</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 12 }}>
        {[
          { icon: <Gift size={16} color="#9055ff" />, bg: "rgba(144,85,255,0.14)", border: "rgba(144,85,255,0.24)", label: "Support", sub: "Help & contact" },
        ].map((row, i, arr) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 13, padding: "13px 16px",
            background: "rgba(255,255,255,0.055)", border: "1px solid rgba(255,255,255,0.09)",
            borderRadius: arr.length === 1 ? 18 : i === 0 ? "18px 18px 6px 6px" : i === arr.length - 1 ? "6px 6px 18px 18px" : "6px",
            marginBottom: i === arr.length - 1 ? 0 : 2, boxShadow: CARD_SHADOW, cursor: "pointer" }}>
            <div style={{ width: 36, height: 36, borderRadius: 11, flexShrink: 0,
              background: row.bg, border: `1px solid ${row.border}`,
              display: "flex", alignItems: "center", justifyContent: "center" }}>{row.icon}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 600, color: "rgba(255,255,255,0.92)", letterSpacing: "-0.2px" }}>{row.label}</div>
      ...
      {tonAddress && (
        <div style={{ borderRadius: 18, overflow: "hidden",
          ...
          <button style={{ width: "100%", padding: "14px 16px", background: "rgba(235,75,75,0.07)",
            ...
            <div style={{ width: 36, height: 36, borderRadius: 11, flexShrink: 0,
              background: "rgba(235,75,75,0.12)", border: "1px solid rgba(235,75,75,0.20)",
              display: "flex", alignItems: "center", justifyContent: "center" }}>
              <LogOut size={16} color="#eb4b4b" />
            </div>
      ...
      <div className="container">
        ...
        {activeTab === "home" && (
          <>
            ...
            {/* ── Hero welcome strip ── */}
            <div style={{ borderRadius: 20, padding: "18px 18px 16px", marginBottom: 22,
              background: "linear-gradient(135deg, rgba(42,171,238,0.14) 0%, rgba(75,105,255,0.10) 100%)",
              border: "1px solid rgba(42,171,238,0.20)", boxShadow: CARD_SHADOW_DEEP,
              position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: -18, right: -18, width: 90, height: 90, borderRadius: "50%",
                background: "radial-gradient(circle, rgba(42,171,238,0.22), transparent 70%)", pointerEvents: "none" }} />
              <div style={{ fontSize: 13, fontWeight: 600, color: "rgba(42,171,238,0.90)", marginBottom: 4, letterSpacing: "-0.1px" }}>
                🎁 Welcome back
              </div>
              <div style={{ fontSize: 19, fontWeight: 800, color: "#fff", letterSpacing: "-0.5px", lineHeight: 1.2, marginBottom: 8 }}>
                Open cases, win rare items
              </div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.42)", fontWeight: 500, lineHeight: 1.5 }}>
                Pay with TON or Telegram Stars. Every case has guaranteed drops.
              </div>
            </div>

            <div className="section-header">
              <div className="section-title">Featured Cases</div>
              {!loadingCases && cases.length > 0 && (
                <div className="section-badge">{cases.length} available</div>
              )}
            </div>
            ...
            {!loadingCases && cases.length > 4 && (
              <button onClick={() => setActiveTab("cases")}
                style={{ width: "100%", marginTop: 14, height: 44, borderRadius: 13,
