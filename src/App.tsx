...
// ─── User upsert (login tracking) ────────────────────────────────────────────
async function upsertTelegramUser(): Promise<UserProfile | null> {
  const tg = tgUser();
  ...
  const { data, error } = await supabase
    ...
    .select("id, telegram_id, username, created_at, cases_opened, stars_balance")
    .single();
  if (error) console.error("[upsertTelegramUser]", error.message);
...
const TON_PRESETS   = ["1", "5", "10", "25", "50", "100"];
const STARS_PRESETS = ["50", "100", "250", "500", "1000", "2500"];

function WalletModal({ mode, onClose, tonAddress, userId, currentStarsBalance, onStarsDeposited }: {
  mode: WalletMode;
  onClose: () => void;
  tonAddress: string;
  userId?: string;
  ...
  const presets    = currency === "TON" ? TON_PRESETS : STARS_PRESETS;
  const isDeposit  = mode === "deposit";
  const accentColor = currency === "TON" ? "#2AABEE" : "#e4ae39";
  const accentAlpha = currency === "TON" ? "rgba(42,171,238," : "rgba(228,174,57,";
  ...
  // ── Stars deposit via Telegram.WebApp.openInvoice ──────────────────────────
  const handleStarsDeposit = async () => {
    ...
    try {
      // Your backend must expose this endpoint: POST /api/stars-invoice { amount }
      // It calls Telegram Bot API createInvoiceLink and returns { invoiceUrl }
      ...
      tg.openInvoice(invoiceUrl, async (status: string) => {
        if (status === "paid") {
          // Update stars_balance in Supabase
          ...
          if (userId) {
            const { error } = await supabase
              ...
              .eq("id", userId);
            if (error) console.error("[stars deposit update]", error.message);
          }
  ...
  const handleSubmit = () => {
    if (isDeposit && currency === "STARS") {
      handleStarsDeposit();
      return;
    }
    // TON deposit / withdraw: plug in TonConnect send tx here
    setStep("done");
    setTimeout(handleClose, 1800);
  };
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.38)", fontWeight: 500, marginTop: 2 }}>
              {isDeposit ? "Add TON or Stars to your balance" : "Send funds to your wallet"}
            </div>
          </div>
        ...
        {step === "processing" ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center",
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", textAlign: "center" }}>
              Complete the payment in Telegram to credit your Stars balance.
            </div>
          </div>
        ) : step === "done" ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center",
            gap: 14, padding: "8px 24px 40px" }}>
            <div style={{
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", letterSpacing: "-0.5px" }}>
              {isDeposit ? "Stars Added!" : "Withdrawal Submitted!"}
            </div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.40)", textAlign: "center", lineHeight: 1.6 }}>
              {isDeposit
                ? "Your Stars balance has been updated."
                : "Funds will arrive in your wallet within a few minutes."}
            </div>
          </div>
        ) : step === "confirm" ? (
          <div style={{ padding: "0 20px 32px", display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 10, padding: "12px 14px",
              ...
              <span style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", fontWeight: 500, lineHeight: 1.5 }}>
                {isDeposit && currency === "STARS"
                  ? "Payment processed securely via Telegram. Stars are credited instantly on success."
                  : isDeposit
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={handleSubmit} style={{
                ...
                boxShadow: isDeposit ? "0 4px 20px rgba(42,171,238,0.30)" : "0 4px 20px rgba(228,174,57,0.28)",
              }}>
          ...
          <div style={{ padding: "0 20px 32px", display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <button key={c} onClick={() => { setCurrency(c); setAmount(""); }} style={{
                  ...
                  <img src={c === "TON" ? TonIcon : StarsIcon}
                    style={{ width: 15, height: 15,
                      filter: c === "STARS" ? STARS_FILTER : "none" }} />
                  {c === "STARS" ? "Stars" : "TON"}
                </button>
              ))}
            </div>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.35)",
                textTransform: "uppercase" as any, letterSpacing: "0.4px", marginBottom: 8 }}>
                Amount
              </div>
              <div style={{
                display: "flex", alignItems: "center", gap: 12, padding: "0 16px",
                height: 60, borderRadius: 16,
                background: "rgba(255,255,255,0.05)", border: `1.5px solid ${accentAlpha}0.30)`,
                boxShadow: `0 0 0 3px ${accentAlpha}0.06)`,
              }}>
                <CurrIcon size={20} />
                <input
                  ...
                  style={{
                    ...
                    color: "#fff", fontSize: 24, fontWeight: 800, letterSpacing: "-0.5px",
                    fontFamily: "inherit",
                  }}
                />
                <span style={{ fontSize: 15, fontWeight: 700, color: accentColor }}>{currency === "STARS" ? "Stars" : currency}</span>
              </div>
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                {presets.map(p => (
                  <button key={p} onClick={() => setAmount(p)} style={{
                    height: 40, borderRadius: 11, border: "none",
                    background: amount === p ? `${accentAlpha}0.16)` : "rgba(255,255,255,0.06)",
                    color: amount === p ? accentColor : "rgba(255,255,255,0.65)",
                    fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit",
                    transition: "all 0.15s",
                    boxShadow: amount === p ? `0 0 0 1px ${accentAlpha}0.35)` : "none",
                  }}>{p}</button>
                ))}
              </div>
            </div>

            {/* Wallet destination for withdraw */}
            {!isDeposit && (
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px",
                background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)",
                borderRadius: 14 }}>
                <Wallet size={14} color="rgba(255,255,255,0.40)" />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.30)", fontWeight: 600, marginBottom: 1 }}>To wallet</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: tonAddress ? "rgba(255,255,255,0.75)" : "rgba(255,255,255,0.30)",
                    fontFamily: "'SF Mono','Fira Code',monospace",
                    overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as any }}>
                    {tonAddress ? `${tonAddress.slice(0,10)}…${tonAddress.slice(-6)}` : "No wallet connected"}
                  </div>
                </div>
              </div>
            )}
              ...
              style={{
                ...
                background: !amount || parseFloat(amount) <= 0
                  ...
                  : isDeposit
                    ? "linear-gradient(135deg, #2AABEE 0%, #229ED9 100%)"
                    : "linear-gradient(135deg, #e4ae39 0%, #c99520 100%)",
                color: !amount || parseFloat(amount) <= 0 ? "rgba(255,255,255,0.30)" : "#fff",
                fontSize: 16, fontWeight: 700,
                cursor: !amount || parseFloat(amount) <= 0 ? "not-allowed" : "pointer",
                fontFamily: "inherit", letterSpacing: "-0.1px",
                transition: "all 0.2s",
                boxShadow: !amount || parseFloat(amount) <= 0 ? "none"
                  : isDeposit ? "0 4px 20px rgba(42,171,238,0.32)" : "0 4px 20px rgba(228,174,57,0.28)",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
              }}>
              {isDeposit ? <ArrowDownLeft size={18} /> : <ArrowUpRight size={18} />}
              {isDeposit ? "Deposit" : "Withdraw"} {amount ? `${amount} ${currency === "STARS" ? "Stars" : currency}` : (currency === "STARS" ? "Stars" : currency)}
            </button>
          </div>
        )}
      </div>
...
// ─── Profile Page ─────────────────────────────────────────────────────────────
function ProfilePage({ tonAddress, appUser, onStarsDeposited }: { tonAddress: string; appUser: UserProfile | null; onStarsDeposited?: (b: number) => void }) {
  const [copied, setCopied]             = useState(false);
  const [showSecurity, setShowSecurity] = useState(false);
  const [walletModal, setWalletModal]   = useState<WalletMode | null>(null);

  // appUser comes from the parent (already upserted on app load)
  ...
  const tonBalance   = "0.00"; // TODO: fetch from TON chain via tonAddress
      ...
      <div style={{ marginBottom: 16, borderRadius: 20, overflow: "hidden",
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <div style={{ borderRadius: 14, padding: "12px 14px",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(42,171,238,0.80)",
                  textTransform: "uppercase", letterSpacing: "0.3px" }}>TON</span>
              </div>
              <div style={{ fontSize: 22, fontWeight: 900, color: "#fff", letterSpacing: "-0.8px", lineHeight: 1 }}>
                {tonBalance}
              </div>
            </div>
            {/* Stars balance */}
            <div style={{ borderRadius: 14, padding: "12px 14px",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}>
                <img src={StarsIcon} style={{ width: 16, height: 16, filter: STARS_FILTER }} />
                <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(228,174,57,0.80)",
                  textTransform: "uppercase", letterSpacing: "0.3px" }}>Stars</span>
              </div>
              <div style={{ fontSize: 22, fontWeight: 900, color: "#fff", letterSpacing: "-0.8px", lineHeight: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
          <button
            ...
            style={{
              display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
              padding: "15px 12px", background: "none", border: "none",
...
export default function App() {
  ...
  // Login / upsert on first render
  useEffect(() => {
    upsertTelegramUser().then(u => { if (u) setAppUser(u); });
  }, []);
  ...
  const starsGlobal = appUser?.stars_balance ?? 0;

  // Called by WalletModal after a successful Stars deposit to refresh balance
  const handleStarsDeposited = (newBalance: number) => {
    setAppUser(u => u ? { ...u, stars_balance: newBalance } : u);
  };
    ...
    <div className="app">
      ...
      <div className="container">
        ...
        <div className="balance-row">
          <div className="pill">
            <img src={StarsIcon} className="stars-icon" />{starsGlobal.toLocaleString()}
          </div>
          <div className="pill">
        ...
        {/* ── PROFILE ── */}
        {activeTab === "profile" && <ProfilePage tonAddress={tonAddress} appUser={appUser} onStarsDeposited={handleStarsDeposited} />}
