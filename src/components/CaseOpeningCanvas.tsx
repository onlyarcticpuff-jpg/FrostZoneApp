...
// ─────────────────────────────────────────────
// Gradient texture helper — creates a 1×H gradient strip
// ─────────────────────────────────────────────
function makeGradientTexture(
  ...
  const rt = PIXI.RenderTexture.create({ width: 1, height: steps });
  const g  = new PIXI.Graphics();

  const rf = (colorFrom >> 16) & 0xff, gf = (colorFrom >> 8) & 0xff, bf = colorFrom & 0xff;
  const rt2 = (colorTo   >> 16) & 0xff, gt = (colorTo   >> 8) & 0xff, bt = colorTo   & 0xff;

  for (let i = 0; i < steps; i++) {
    const t  = i / (steps - 1);
    const r  = Math.round(rf + (rt2 - rf) * t);
    const gr = Math.round(gf + (gt  - gf) * t);
    const b  = Math.round(bf + (bt  - bf) * t);
    const a  = alphaFrom + (alphaTo - alphaFrom) * t;
    g.rect(0, i, 1, 1);
  ...
  // Scale texture to actual height
  const final = PIXI.RenderTexture.create({ width: 1, height });
  return rt; // caller scales the sprite
}

// ─────────────────────────────────────────────
// Card builder — clean, no drop shadow
// ─────────────────────────────────────────────
async function buildCard(
  ...
  bg.fill({ color: 0x141720, alpha: 1 });
  card.addChild(bg);

  // ── Subtle rarity tint overlay ─────────────
  const tint = new PIXI.Graphics();
  tint.roundRect(0, 0, CARD_W, CARD_H, 4);
  tint.fill({ color: col, alpha: 0.055 });
  card.addChild(tint);

  // ── Top rarity bar — 3px, full-brightness ──
  const bar = new PIXI.Graphics();
  bar.roundRect(0, 0, CARD_W, 3, 2);
  bar.fill({ color: col, alpha: 1 });
  card.addChild(bar);

  // ── Crisp border — single-pixel, rarity col ─
  const border = new PIXI.Graphics();
  border.roundRect(0.5, 0.5, CARD_W - 1, CARD_H - 1, 4);
  border.stroke({ color: col, alpha: 0.35, width: 1 });
  card.addChild(border);

  // ── Image area ─────────────────────────────
  const imgArea = new PIXI.Graphics();
  imgArea.roundRect(10, 12, CARD_W - 20, 90, 3);
  imgArea.fill({ color: 0x0a0c10, alpha: 1 });
  card.addChild(imgArea);

  // ── Gem / image placeholder ────────────────
  const gem = new PIXI.Graphics();
  // Outer glow ring
  gem.circle(CARD_W / 2, 57, 20);
  gem.fill({ color: col, alpha: 0.12 });
  // Inner filled circle
  gem.circle(CARD_W / 2, 57, 12);
  gem.fill({ color: col, alpha: 0.7 });
  // Highlight dot
  gem.circle(CARD_W / 2 - 4, 53, 3);
  ...
  if (item.image_url) {
    try {
      ...
      sprite.x = CARD_W / 2;
      sprite.y = 57;
      sprite.scale.set(Math.min(76 / tex.width, 70 / tex.height));
  ...
  div.rect(10, 107, CARD_W - 20, 1);
  div.fill({ color: col, alpha: 0.15 });
  card.addChild(div);
